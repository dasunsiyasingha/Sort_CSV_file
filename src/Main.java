...
public class Main extends Application { 
    ...
    public void start(Stage primaryStage) { 
        ...
        selecFile.setStyle("-fx-background-color: #A9CCF9; -fx-padding: 
...
// --- Sorting Results Blocks --- 
        VBox block1 = createSortBlock("Insertion Sort", 
textAreaInsertion, execTimeInsertion); 
        VBox block2 = createSortBlock("Bubble Sort", textAreaBubble, 
execTimeBubble); 
        VBox block3 = createSortBlock("Merge Sort", textAreaMerge, 
execTimeMerge); 
        VBox block4 = createSortBlock("Quick Sort", textAreaQuick, 
execTimeQuick); 
        VBox block5 = createSortBlock("Heap Sort", textAreaHeap, 
execTimeHeap); 
        ...
        Label bestAlgo = new Label("Identify and display the best
performing algorithm based on the shortest execution time."); 
...
12; -fx-background-radius: 10; -fx-font-size: 14;"); 
        ...
        btnBrowse.setOnAction(e -> { 
            fileUploadOk = fileUploadValidate.fileUpload(primaryStage, 
errorMsg); 
 
            if(fileUploadOk){ 
                fileValidatedOk = 
fileUploadValidate.fileValidate(columnSelector, errorMsg); 
 
                if(fileValidatedOk){ 
                    fileUploadValidate.loadCSVColumns(columnSelector, 
errorMsg); 
                } 
            } 
        ...
        btnSort.setOnAction(e -> { 
            if(columnSelector.getSelectionModel().getSelectedIndex() != 
1){ 
                errorMsg.setVisible(false); 
                CSVFileData csvFileInfo = 
fileUploadValidate.validateNumericColumn(columnSelector.getSelectionModel
().getSelectedIndex(), errorMsg); 
                if(csvFileInfo != null){ 
                    ExtractData extractData = new 
ExtractData(csvFileInfo.file, csvFileInfo.Index); 
    ...
    private VBox createSortBlock(String title, TextArea textArea, Label 
execTime) { 
